  ...
  const fetchEmployee = async () => {
    try {
      ...
      const countryCounts = employees.reduce((acc, employee) => {
        acc[employee.department.departmentName]= (acc[employee.department.departmentName] || 0) + 1;
        return acc;
      }, {});

      const totalEmployees = employees.length;

      let pieData = Object.keys(countryCounts).map((departmentName, index) => ({
        id: index,
        label: departmentName,
        value: (countryCounts[departmentName] / totalEmployees) * 100,
      }));
      ...
      if (otherValue > 0) {
        top5Data.push({ id: 6, label: 'Other', value: otherValue });
      }
