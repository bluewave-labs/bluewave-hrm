...
export default function EmployeesNationalityGraph() {
  ...
  const fetchEmployee = async () => {
    try {
      ...
      const nationalityCounts = employees.reduce((acc, employee) => {
        acc[employee.nationality]= (acc[employee.nationality] || 0) + 1;
        return acc;
      }, {});

      const totalEmployees = employees.length;

      let pieData = Object.keys(nationalityCounts).map((nationality, index) => ({
        id: index,
        label: nationality,
        value: (nationalityCounts[nationality] / totalEmployees) * 100,
      }));
